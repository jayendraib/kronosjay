"""
Hourly counterpart to run_walkforward.py (daily). Same Filtered+Hysteresis
strategy and trade-log schema, same 5 symbols and 2019-01-01 scoring start,
just at hourly granularity (context_len=168 bars = 7 days, block_len=24
bars = 1 day, matching the hourly config used elsewhere in this repo).

Reads {symbol}_1h_raw.csv (from fetch_multi_coin_data_1h.py, which starts
2018-11-01 for buffer). Writes {symbol}_1h_results.csv, {symbol}_1h_trades.csv,
and a combined all_coins_trade_log_1h.csv -- all separate from the daily
_1d_ outputs so neither run clobbers the other.

This is ~5x the blocks-per-coin of the daily run (roughly 2500+ vs ~550),
so expect a meaningfully longer wall-clock time; progress is printed per
symbol and per prediction-batch chunk.

Run with:
    .venv/bin/python examples/rl_trading/run_walkforward_1h.py
"""
import os
import sys
import time
...
def run_symbol_forecast(predictor, symbol):
    raw_path = os.path.join(OUT_DIR, f"{symbol}_1h_raw.csv")
    ...
    print(f"  [{symbol}] {num_blocks} blocks x {BLOCK_LEN} bars (context={CONTEXT_LEN})", flush=True)
    ...
    for start in range(0, num_blocks, PREDICT_BATCH_SIZE):
        ...
        if done % (PREDICT_BATCH_SIZE * 10) == 0 or done == num_blocks:
            print(f"    [{symbol}] {done}/{num_blocks} blocks forecast ({elapsed:.0f}s elapsed)", flush=True)
    ...
    print(f"  [{symbol}] scored {len(result)} bars, {result['timestamp'].min()} -> {result['timestamp'].max()}", flush=True)
...
def main():
    ...
    print("Loading Kronos-small + Kronos-Tokenizer-base ...", flush=True)
    ...
    for symbol in SYMBOLS:
        ...
        df.to_csv(os.path.join(OUT_DIR, f"{symbol}_1h_results.csv"), index=False)

        trades = extract_trades(df, symbol, SLIPPAGE_PCT)
        trades.to_csv(os.path.join(OUT_DIR, f"{symbol}_1h_trades.csv"), index=False)
        ...
        print(f"  {symbol}: {num_trades} trades, win_rate={win_rate:.1f}%, "
              ...
              f"took {elapsed / 60:.1f} min", flush=True)
        summary_rows.append({
    ...
    combined.to_csv(os.path.join(OUT_DIR, "all_coins_trade_log_1h.csv"), index=False)
    ...
    print(f"Combined trade log: {os.path.join(OUT_DIR, 'all_coins_trade_log_1h.csv')}")
